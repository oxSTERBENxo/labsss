...
public class SkiSlopeController {
    ...
    public String listAll(@RequestParam(required = false) String name,
        ...
        Page<SkiSlope> page =
                skiSlopeService.findPage(name, length, difficulty, skiResort, pageNum - 1, pageSize);

        // list.html uses ${skiSlopes} in th:each
        model.addAttribute("skiSlopes", page.getContent());

        // optional, useful for pagination if you add it later
    ...
    public String showEdit(@PathVariable Long id, Model model) {
        SkiSlope slope = skiSlopeService.findById(id);
